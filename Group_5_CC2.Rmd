...
library(ggplot2)
visualization = ggplot(aes(x = ovary_area, fill = population_type),data=cc1_data) + geom_histogram(binwidth = 1) + theme_classic() + xlab('Ovary Area (mm2)') + ylab('Count') + labs(title = 'Ovary Area by Population Type in Solanum pimpinellifolium', fill = NULL) + scale_fill_discrete(labels = c('Asexual','Sexual'))
