...
1. **Zeshan Ghuman** — 20479122
    + [BIOL343_CC2](https://github.com/Potato9715/BIOL343_CC2)
1. **Allie Goodyear** - 20443234
    + [BIOL343_CC2](https://github.com/alliegoodyear/BIOL343_CC2)
1. **Coralie Spakowski** - 20471806
    + [BIOL343_CC2](https://github.com/coraliespakowski/BIOL343_CC)
1. **Jake Mayo** - 20186062
    + [BIOL343_CC2](https://github.com/jake-mayo/BIOL343.CC)
1. **Noah Lu** - 20483156
    + [BIOL343_CC2](https://github.com/NLRcode/BIOL343_CC)
1. **Iryna Ozimek** - 20508451
    + [BIOL343_CC2](https://github.com/irynaoz323/BIOL343.CC)
...
---------|--------------------------------|--------------------------------------------------------
population_type | Indicates whether the population is asexual or sexual | No Units |
ovary_width | A measure of the width of the ovaries which is a reliable measure of overall flower size | mm |
ovary_height | A measure of the height of the ovaries which is a reliable measure of overall flower size | mm |
flower_number | The average number of flowers produced by each individual in a sample of plants from each population | Count |
...
```{r import-clean}
cc1_data <- read.csv("./data/A1_Data_Excel.csv")
cc1_data <- cc1_data[1:46,1:8]
names(cc1_data) = c('population_type','population_name','latitude','longitutde','ovary_width','ovary_height','average_august_temperature','flower_number')
cc1_data$average_august_temperature = as.numeric(cc1_data$average_august_temperature)
cc1_data$ovary_area <-  cc1_data$ovary_height * cc1_data$ovary_width
...
$$Ovary\:Area = Ovary\:Width \times Ovary\:Height$$
...
```{r area-means, echo=F}
area_means <- aggregate(ovary_area ~ population_type, data = cc1_data, FUN = mean)
```
...
```{r area-table, echo=FALSE, warning=FALSE, message=FALSE}
library(dplyr)
library(knitr)
AreaTable <- area_means %>% 
    
    group_by(population_type)
kable(AreaTable, caption = "Ovary area means grouped by population types (Asexual and Sexual)")
```
...
library(ggplot2)
visualization = ggplot(aes(x = ovary_area, fill = population_type),data=cc1_data) + geom_histogram(binwidth = 1) + theme_classic() + xlab('Ovary Area (mm^2)') + ylab('Count') + labs(title = 'Ovary Area by Population Type in Solanum pimpinellifolium', fill = NULL) + scale_fill_discrete(labels = c('Asexual','Sexual'))
visualization + facet_wrap(.~population_type)
```
...
![Beautiful photo of Solanum pimpinellifolium](./images/Hydroponic_g13.jpg)

[Source](https://onq.queensu.ca/d2l/le/dropbox/1208667/492096/DownloadAttachment?fid=54136518)
